...
import {
    UpdateUploadedArtworksResponseBody,
    RequestValidationSchema,
    UpdateUploadedArtworksRequestBody
} from './apiSchema';
...
import {getRecaptchaSecret, getTezosActivatorAccount} from '../../../common/SecretsManager';
...
const baseHandler = async (event): Promise<LambdaResponseToApiGw> => {
    ...
    const captchaSecret = await getRecaptchaSecret();
    const secret: string = captchaSecret.recaptcha;

    const token = body.token;

    let response = await fetch(`https://www.google.com/recaptcha/api/siteverify?` + new URLSearchParams({
        secret: secret,
        response: token
    }));
    let captchaResponse = await response.json();
    console.log(captchaResponse);

    if (captchaResponse.success && captchaResponse.action === 'postWalletId' && captchaResponse.score >= 0.5) {
        console.log('captcha solved correctly');
    } else {
        returnObject = 'Captcha Error: ' + captchaResponse['error-codes'];
